...
public class ActionDaoImpl implements ActionDao {
	...
	public boolean update(Action action) {
		try {
			PreparedStatement preparedStatement = this.connection.prepareStatement("update action set name=?, opening_amount=?, closing_amount=?, date=?, bourse_id=? where id=?");	
			preparedStatement.setString(1, action.getName());
			preparedStatement.setDouble(2, action.getOpeningAmount());
			preparedStatement.setDouble(3, action.getClosingAmount());
			preparedStatement.setDate(4, (Date) action.getDate());
	...
	public Collection<Action> findByBourseId(Long id) {
		...
		try {
			...
			PreparedStatement preparedStatement = this.connection.prepareStatement("select *from action where bourse_id=?");	
			preparedStatement.setLong(1, id);
			Collection<Action> actions = new ArrayList<Action>();
			ResultSet resultSet = preparedStatement .executeQuery();
			while( resultSet.next() ) {
